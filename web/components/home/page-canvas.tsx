 ...
 * different rates as you scroll (layered parallax depth), with a fine grain
 * over the top for tactile realism. Sits fixed behind all content; transparent
 * sections show it through. Honors prefers-reduced-motion (renders static).
 */
export function PageCanvas() {
  const reduce = useReducedMotion();
  const { scrollYProgress } = useScroll();

  // Distinct drift distances/directions → the blooms separate as you scroll.
  ...
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      ...
      />
      {/* fine grain - analog tactility / depth, won't hurt text contrast */}
